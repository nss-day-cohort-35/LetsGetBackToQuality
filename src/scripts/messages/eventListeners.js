const messageEventListener = {
	...
	setEdit: function(event) {
		...
		const name = document
			.querySelector(`#message-${splitID[1]} .message-name`)
			.innerText.split(":");
		const date = document
		...
		document.querySelector("#message-box").value = document.querySelector(
			`#message-${splitID[1]} .message-value`
		).innerText;
	},

	putEdit: function(userIdNum) {
		const dateSplit = document
			.querySelector(
				`#message-${
					document.querySelector("#message-number").value
				} .message-date`
			)
			.innerText.split(":");
		const userIdNumParsed = parseInt(userIdNum);
		const editedChat = {
